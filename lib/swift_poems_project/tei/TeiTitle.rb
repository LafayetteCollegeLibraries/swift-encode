...
=end
    ...
    def pushToken(token)
      ...
      if NotaBene::NB_MARKUP_TEI_MAP.has_key? @current_leaf.name

        # Does this seem to close the current leaf?
        if NotaBene::NB_MARKUP_TEI_MAP[@current_leaf.name].has_key? token
...
#            @header.poem.link_group.add_link target, source
          ...
          if @current_leaf.class.to_s == 'Nokogiri::XML::Element'
            ...
            closed_tag_name = NotaBene::NB_MARKUP_TEI_MAP[@current_leaf.name][token].keys[0]

            NotaBene::NB_MARKUP_TEI_MAP[@current_leaf.name][token][closed_tag_name].each_pair do |attrib_name, attrib_value|
          ...
          # @todo Resolve
          if NotaBene::NB_MARKUP_TEI_MAP.has_key? opened_tag.name and NotaBene::NB_MARKUP_TEI_MAP[opened_tag.name] != nil

            while opened_tag and NotaBene::NB_MARKUP_TEI_MAP.has_key? opened_tag.name and NotaBene::NB_MARKUP_TEI_MAP[opened_tag.name].has_key? token
          ...
          if NotaBene::NB_DELTA_FLUSH_TEI_MAP.has_key? token

            current_leaf = NotaBene::FlushDelta.new(token, @document, @current_leaf)
          elsif NotaBene::NB_DELTA_ATTRIB_TEI_MAP.has_key? token

            current_leaf = NotaBene::AttributeNotaBeneDelta.new(token, @document, @current_leaf)
          else

            current_leaf = NotaBene::UnaryNotaBeneDelta.new(token, @document, @current_leaf)
          end
        else
          ...
          # @todo Refactor
          @current_leaf = NotaBene::BinaryNotaBeneDelta.new(token, @document, @current_leaf)
        ...
        # newLeaf = Nokogiri::XML::Node.new NotaBene::NB_SINGLE_TOKEN_TEI_MAP[token].keys[0], @document
    ...
    def push(token)
      ...
      is_a_closing_ternary_token = NotaBene::NB_TERNARY_TOKEN_TEI_MAP.has_key?(@current_leaf.name) && NotaBene::NB_TERNARY_TOKEN_TEI_MAP[@current_leaf.name][:secondary].has_key?(token)
      
      is_a_closing_token = NotaBene::NB_MARKUP_TEI_MAP.has_key?(@current_leaf.name) && NotaBene::NB_MARKUP_TEI_MAP[@current_leaf.name].has_key?(token)
      
      if NotaBene::NB_SINGLE_TOKEN_TEI_MAP.has_key? token or is_a_closing_ternary_token or is_a_closing_token or NotaBene::NB_MARKUP_TEI_MAP.has_key? token
        pushToken token
      else
        pushText token
      end
