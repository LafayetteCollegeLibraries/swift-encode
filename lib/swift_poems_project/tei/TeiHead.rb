...
#      @heads.elem.add_child @elem
    ...
    def pushToken(token)
      ...
      # @todo Refactor
      if NotaBene::NB_MARKUP_TEI_MAP.has_key? @current_leaf.name and not /«FN./.match(token)

        if NotaBene::NB_TERNARY_TOKEN_TEI_MAP.has_key? @current_leaf.name and NotaBene::NB_TERNARY_TOKEN_TEI_MAP[@current_leaf.name][:secondary].has_key? token
          ...
          @current_leaf.name = NotaBene::NB_TERNARY_TOKEN_TEI_MAP[@current_leaf.name][:secondary][token].keys[0]
        ...
        elsif NotaBene::NB_MARKUP_TEI_MAP[@current_leaf.name].has_key? token # If this token closes the currently opened token
          ...
          attribMap = NotaBene::NB_MARKUP_TEI_MAP[@current_leaf.name][token].values[0]
          @current_leaf[attribMap.keys[0]] = attribMap[attribMap.keys[0]]

          # One cannot resolve the tag name and attributes until both tags have been fully parsed
          @current_leaf.name = NotaBene::NB_MARKUP_TEI_MAP[@current_leaf.name][token].keys[0]
          @current_leaf = @current_leaf.parent
          ...
          while not opened_tag.nil? and NotaBene::NB_MARKUP_TEI_MAP.has_key? opened_tag.name and NotaBene::NB_MARKUP_TEI_MAP[opened_tag.name].has_key? token

            closed_tag = @heads.opened_tags.shift
            closed_tag_name = NotaBene::NB_MARKUP_TEI_MAP[closed_tag.name][token].keys[0]

            # @todo Integrate Nota Bene Delta Objects
            NotaBene::NB_MARKUP_TEI_MAP[closed_tag.name][token][closed_tag_name].each_pair do |attrib_name, attrib_value|
        ...
        elsif NotaBene::NB_MARKUP_TEI_MAP.has_key? token
          ...
          @heads.opened_tags.unshift @current_leaf
        elsif NotaBene::NB_SINGLE_TOKEN_TEI_MAP.has_key? token

          newLeaf = Nokogiri::XML::Node.new token, @document

          newLeaf.name = NotaBene::NB_SINGLE_TOKEN_TEI_MAP[token].keys[0]

          NotaBene::NB_SINGLE_TOKEN_TEI_MAP[token][newLeaf.name].each do |name, value|
        ...
        @current_leaf.name = NotaBene::NB_SINGLE_TOKEN_TEI_MAP[token].keys[0]
    ...
    def pushText(token)
      ...
      NotaBene::NB_ASCII_SEQS.each do |ascii_seq, utf8_seq|
    ...
    def push(token)
      ...
      if token == '«MDNM»' and /«FN./.match(@current_leaf.name) and @heads.opened_tags.length >= 2
        ...
        # @todo Refactor
        closed_tag_name = NotaBene::NB_MARKUP_TEI_MAP[@heads.opened_tags[1].name][token].keys[0]

        # @todo Integrate Nota Bene Delta Objects
        NotaBene::NB_MARKUP_TEI_MAP[@heads.opened_tags[1].name][token][closed_tag_name].each_pair do |attrib_name, attrib_value|
      ...
      elsif NotaBene::NB_SINGLE_TOKEN_TEI_MAP.has_key? token or
          (NotaBene::NB_TERNARY_TOKEN_TEI_MAP.has_key? @current_leaf.name and NotaBene::NB_TERNARY_TOKEN_TEI_MAP[@current_leaf.name][:secondary].has_key? token) or
          (NotaBene::NB_MARKUP_TEI_MAP.has_key? @current_leaf.name and NotaBene::NB_MARKUP_TEI_MAP[@current_leaf.name].has_key? token) or
          NotaBene::NB_MARKUP_TEI_MAP.has_key? token
