...
require 'fileutils'
require "#{File.dirname(__FILE__)}/lib/swift_poems_project"

config = ParseConfig.new(File.join(File.dirname(__FILE__), 'config/server.conf').chomp)

# Configuration for Nota Bene parsing
...
# Tasks in Thor
class SwiftPoemsProject < Thor
  package_name "SwiftPoemsProject"
  ...
  desc "clear_caches", "clear the cache for the Nota Bene and TEI transcripts"
  def clear_caches

    Dir.glob(File.join(NB_CACHE_PATH,'**')).each do |filename|
      FileUtils.rm filename
    end
    $stdout.puts "Nota Bene cache successfully cleared"

    Dir.glob(File.join(TEI_CACHE_PATH,'**', '*.tei.xml')).each do |filename|
      FileUtils.rm filename
    end

    $stdout.puts "TEI cache successfully cleared"
  end

  option :source, :desc => 'Identifier for the source', :aliases => "-s", :required => true
  option :transcript_id, :desc => 'Identifier for the transcript', :aliases => "-t", :required => true
  desc "encode_transcript", "encode a transcript into the TEI-P5"
  def encode_transcript

    @nota_bene_store = ::SwiftPoemsProject::GDrive::NotaBeneStore.new(CLIENT_SECRETS_PATH, SCOPE, APPLICATION_NAME, NB_CACHE_PATH)
    @encoder = ::SwiftPoemsProject::Encoder::TeiEncoder.new(TEI_CACHE_PATH)
    
    result = @nota_bene_store.transcript(options[:transcript_id], options[:source])
    transcript_content = result[:content].encode('utf-8','cp437')
    transcript_xml_content = @encoder.encode(options[:source], options[:transcript_id], transcript_content, result[:mtime])

    $stdout.puts "Transcript successfully encoded in #{File.join(TEI_CACHE_PATH, options[:transcript_id] + '.tei.xml')}"
  end
